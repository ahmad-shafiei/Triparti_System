...
data3a = np.column_stack([xarray, bblack,red ,blue])        ####
np.savetxt("Fig4a.txt" , data3a, fmt=['%s','%s','%s','%s'])    ###

linestyle =['-','--','--','-']
color=['black', 'black', 'r', 'b']
fig, ax= plt.subplots(1,1,figsize=(6,4))
for i in [1,2,3]:
    plt.plot(xarray, data3a[:,i], linestyle = linestyle[i], color = color[i]) #####
